...
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_ALWAYS_ASYNC_FS;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_CACHE_ENTRY_TIMEOUT;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_CACHING_ENABLED;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_DIRECTORY_LISTING;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_DIRECTORY_TEMPLATE;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_ENABLE_FS_TUNING;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_FILES_READ_ONLY;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_INCLUDE_HIDDEN;
...
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_MAX_AVG_SERVE_TIME_NS;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_MAX_CACHE_SIZE;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_RANGE_SUPPORT;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_ROOT_FILESYSTEM_ACCESS;
import static io.vertx.ext.web.handler.StaticHandler.DEFAULT_SEND_VARY_HEADER;
...
public class HomeStoreHandler implements StaticHandler {

    @Override
    public void handle(RoutingContext ctx) {
        HttpServerRequest req = ctx.request();
        req.headers().remove("accept");
        req.headers().add("accept", "json");
        ctx.normalisedPath();
    ...
    private String getRealPath(String p) {
        return p; //TODO
    }
//////////////////////////////////////////////////////////////
  private static final io.vertx.core.logging.Logger log = LoggerFactory.getLogger(StaticHandlerImpl.class);
  ...
  public HomeStoreHandler(String root, ClassLoader classLoader) {
    this.classLoader = classLoader;
    setRoot(root);
  }

  public HomeStoreHandler() {
    classLoader = null;
  }
  ...
  private String getFile(String path, RoutingContext context) {
    String file = webRoot + getRealPath(Utils.pathOffset(path, context));
  ...
  private void readDir(final RoutingContext ctx, final String p, final String filter, final Handler<AsyncResult<List<String>>> handler) {
    ctx.vertx().<List<String>>executeBlocking(feature -> {
        try {
            //TODO read from db
          File file = new FileResolver(ctx.vertx()).resolveFile(p);
          if (!file.exists()) {
  ...
  private void sendDirectoryListing(String dir, RoutingContext context) {
    ...
    readDir(context, dir, null, asyncResult -> {
